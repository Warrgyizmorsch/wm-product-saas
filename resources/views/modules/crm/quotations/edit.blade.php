                                    ...
                                    @foreach($quotation->items as $idx => $item)
                                        <tr class="item-row">
                                            ...
                                            <td class="text-end fw-bold item-amount pe-3" style="padding-top: 8px;">
                                                {{ currency_symbol() }}{{ number_format(($item->quantity * $item->unit_price) * (1 + (($item->tax_rate ?: 18) / 100)), 2) }}
                                            </td>
                        ...
                        <div class="col-md-5">
                            <div class="pe-md-2">
                                <div class="d-flex justify-content-between align-items-center py-2 border-bottom">
                                    ...
                                    <span class="fw-bold text-dark fs-14" id="calcSubtotal">{{ currency_symbol() }}{{ number_format($quotation->subtotal, 2) }}</span>
                                </div>

                                <div class="d-flex justify-content-between align-items-center py-2 border-bottom">
                                    <span class="text-muted fw-semibold fs-13">{{ __('crm.taxes') }}:</span>
                                    <span class="fw-bold text-dark fs-14" id="calcTax">{{ currency_symbol() }}{{ number_format($quotation->tax, 2) }}</span>
                                </div>
                                ...
                                <div class="d-flex justify-content-between align-items-center py-3 border-top border-2 mt-3" style="border-top: 2px solid var(--bs-primary) !important;">
                                    ...
                                    <span class="fw-extrabold text-primary fs-20" id="calcTotal">{{ currency_symbol() }}{{ number_format($quotation->total_amount, 2) }}</span>
                                </div>
                            </div>
                        </div>
                    </div>
        ...
        $(document).ready(function() {
            ...
            function calculateTotals() {
                ...
                $('#itemsTable tbody tr').each(function() {
                    ...
                    $(this).find('.item-amount').text('{{ currency_symbol() }}' + (lineTotal + lineTax).toLocaleString('en-IN', {minimumFractionDigits: 2, maximumFractionDigits: 2}));
                });

                var discount = parseFloat($('#discountInput').val()) || 0;
                var grandTotal = Math.max(0, subtotal + totalTax - discount);

                $('#calcSubtotal').text('{{ currency_symbol() }}' + subtotal.toLocaleString('en-IN', {minimumFractionDigits: 2, maximumFractionDigits: 2}));
                $('#calcTax').text('{{ currency_symbol() }}' + totalTax.toLocaleString('en-IN', {minimumFractionDigits: 2, maximumFractionDigits: 2}));
                $('#calcTotal').text('{{ currency_symbol() }}' + grandTotal.toLocaleString('en-IN', {minimumFractionDigits: 2, maximumFractionDigits: 2}));
            }
