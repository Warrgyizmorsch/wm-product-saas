...
<body>
    ...
    <div class="cert-container">
        ...
        <div class="cert-header d-flex justify-content-between align-items-center">
            <div>
                <h3 class="fw-bold text-success mb-0">{{ $document->content_data['company_name'] ?? 'Warrgyizmorsch Pvt Ltd' }}</h3>
        ...
        <h4 class="cert-title">TO WHOMSOEVER IT MAY CONCERN</h4>
        ...
        <div class="cert-body fs-15 lh-lg">
            <p class="mb-4">
                ...
                was employed with <strong>{{ $document->content_data['company_name'] }}</strong> from 
                <strong>{{ $document->content_data['date_of_joining'] }}</strong> to <strong>{{ $document->content_data['last_working_day'] }}</strong> 
                (Total Tenure: <strong>{{ $document->content_data['tenure_string'] }}</strong>).
            </p>

            <p class="mb-4">
                During their tenure with our organization, they held the position of 
                <strong>{{ $document->content_data['designation'] }}</strong> in the 
                <strong>{{ $document->content_data['department'] }}</strong> department.
            </p>

            <p class="mb-4">
                {{ $document->content_data['conduct_statement'] ?? 'During their service with us, we found them to be sincere, hardworking, dedicated, and professionally competent in the discharge of their duties.' }}
            </p>

            <p class="mb-4">
                They have completed all exit formalities, and we wish them continued success in all their future professional endeavors.
            </p>
        </div>
        ...
        <div class="signature-section d-flex justify-content-between align-items-end pt-5">
            <div>
                ...
                <div class="fw-bold fs-15 text-dark">Head of Human Resources</div>
