...
@section('content')
    ...
    <script>
        document.addEventListener("DOMContentLoaded", function() {
            ...
            // Correctly initialize Select2 status selector inside addCompanyModal when modal is shown (avoiding width: 0px bug)
            $('#addCompanyModal').on('shown.bs.modal', function () {
                var $statusSelect = $(this).find('[data-select2-selector="status"]');
                if ($statusSelect.length) {
                    if ($statusSelect.hasClass("select2-hidden-accessible")) {
                        $statusSelect.select2('destroy');
                    }
                    if (typeof bgformat === 'function') {
            ...
            // Correctly initialize Select2 inside Salary Component modals when shown
