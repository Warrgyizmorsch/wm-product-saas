...
@section('page-actions')
    <div class="d-flex align-items-center gap-2">
        <x-ui.button href="{{ route('production.wip.index') }}" variant="light" class="border px-3" icon="feather-arrow-left">
        ...
        @if($wip->status !== 'completed' && $wip->available_quantity > 0)
            ...
            <x-ui.button variant="light" class="border" data-bs-toggle="modal" data-bs-target="#adjustWipModal" icon="feather-sliders">
                {{ __('production.adjust') }}
            </x-ui.button>
        @endif
        @if($wip->status === 'completed' || $wip->available_quantity > 0)
            <x-ui.button variant="primary" data-bs-toggle="modal" data-bs-target="#convertToFgModal" icon="feather-check-circle">
...
@section('content')
    <div class="erp-single-panel">
        ...
        <div class="row g-4 text-dark fs-13 mb-4">
                    ...
                    <div class="card-body">
                        <div class="row g-3">
                            <div class="col-6">
                                ...
                                <a href="{{ route('production.orders.show', $wip->production_order_id) }}" class="fw-bold text-primary">
                                    {{ $wip->order->order_number }}
                                </a>
                            </div>
                            <div class="col-6">
                                <span class="text-muted d-block fs-11 uppercase font-semibold">Batch / Lot Reference</span>
                                <span class="fw-semibold text-dark">{{ $wip->batch ? $wip->batch->batch_number : 'Direct Order Lot' }}</span>
                            </div>
                            ...
                            <div class="col-6 pt-2 border-top">
                                ...
                                <span class="fw-semibold text-dark">{{ $wip->currentWorkCenter ? $wip->currentWorkCenter->name : 'N/A' }}</span>
                            </div>
                            <div class="col-6 pt-2 border-top">
                                <span class="text-muted d-block fs-11 uppercase font-semibold">Available Qty</span>
                                <span class="fw-bold text-dark fs-16">{{ number_format($wip->available_quantity, 2) }}</span>
                            </div>
                            <div class="col-6 pt-2 border-top">
                                <span class="text-muted d-block fs-11 uppercase font-semibold">Completed Qty</span>
                                <span class="fw-bold text-success fs-16">{{ number_format($wip->completed_quantity, 2) }}</span>
                            </div>
                            <div class="col-6 pt-2 border-top">
                                <span class="text-muted d-block fs-11 uppercase font-semibold">Rejects & Scrap</span>
                                <span class="fw-bold text-danger">{{ number_format($wip->rejected_quantity, 2) }} Rejects</span> | 
            ...
            <div class="col-md-5">
                <div class="card border-0 shadow-sm h-100">
                    <div class="card-header bg-transparent border-bottom pt-4 pb-3">
                        <h5 class="fw-bold mb-0"><i class="feather-pie-chart text-success me-2"></i> {{ __('production.wip_valuation_breakdown') }}</h5>
                    </div>
                    <div class="card-body">
                        <ul class="list-group list-group-flush">
                            ...
                            <li class="list-group-item d-flex justify-content-between align-items-center px-0 pb-0 border-0 bg-soft-primary rounded p-3 mt-3">
                                <div>
            ...
            <div class="col-12">
                <div class="card border-0 shadow-sm">
                    <div class="card-header bg-transparent border-bottom pt-4 pb-3">
                        <h5 class="fw-bold mb-0"><i class="feather-activity text-info me-2"></i> {{ __('production.transaction_history') }}</h5>
                    </div>
                            ...
                            <x-ui.odoo-form-ui type="table">
                                ...
                                <tbody>
                                    @forelse($wip->transactions as $tx)
                                        <tr>
                                            <td class="text-muted font-monospace">{{ $tx->transaction_at->format('d/m/Y H:i:s') }}</td>
                                            <td>
                                                @php
                                                ...
                                                @endphp
                                                <span class="badge {{ $badgeClass }}">{{ strtoupper(str_replace('_', ' ', $tx->transaction_type)) }}</span>
                                            </td>
                                            <td>
                                                <span class="font-monospace text-muted">{{ $tx->fromOperation ? $tx->fromOperation->name : '—' }}</span>
                                            </td>
                                            <td>
                                                <span class="font-monospace text-dark">{{ $tx->toOperation ? $tx->toOperation->name : '—' }}</span>
                                            </td>
    ...
    {{-- Transfer WIP Modal --}}
    @if($wip->status !== 'completed' && $wip->available_quantity > 0)
        <x-ui.modal id="transferWipModal" title="{{ __('production.transfer_wip') }}" formAction="{{ route('production.wip.transfer', $wip->id) }}" submitText="Transfer WIP" closeText="Cancel">
            ...
            <x-ui.odoo-form-ui type="input" inputType="number" label="Qty to Move" name="quantity" step="0.0001" :value="$wip->available_quantity" :max="$wip->available_quantity" :helperText="'Maximum transfer quantity is ' . number_format($wip->available_quantity, 2) . ' units.'" required />

            <x-ui.odoo-form-ui type="input" label="Remarks" name="remarks" placeholder="Optional comments..." />
        </x-ui.modal>

        {{-- Adjust WIP Modal --}}
        <x-ui.modal id="adjustWipModal" title="{{ __('production.adjust_wip') }}" formAction="{{ route('production.wip.adjust', $wip->id) }}" submitText="Apply Adjustment" closeText="Cancel">
            <x-ui.odoo-form-ui type="input" label="Current Qty" name="current_quantity_dummy" :value="number_format($wip->quantity, 4)" readonly />
            
            <x-ui.odoo-form-ui type="input" inputType="number" label="New Qty" name="quantity" step="0.0001" :value="$wip->quantity" required />
    ...
    {{-- Convert WIP Modal --}}
    @if($wip->status === 'completed' || $wip->available_quantity > 0)
        <x-ui.modal id="convertToFgModal" title="{{ __('production.convert_to_fg') }}" formAction="{{ route('production.wip.convert', $wip->id) }}" submitText="Complete FG Inflow" closeText="Cancel">
            <x-ui.odoo-form-ui type="input" label="Convert Qty" name="convert_qty_dummy" :value="number_format($wip->available_quantity, 4) . ' units'" readonly />
