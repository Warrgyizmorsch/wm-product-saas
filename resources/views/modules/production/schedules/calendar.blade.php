            ...
            <div class="d-flex flex-wrap justify-content-between align-items-center mb-4 pb-3 border-bottom gap-3">
                ...
                <div class="d-flex flex-wrap align-items-center gap-2">
                    ...
                    <div class="d-flex align-items-center gap-1 me-2">
                        ...
                        <a href="{{ request()->fullUrlWithQuery(['layout' => 'list']) }}" class="btn btn-sm {{ $layout === 'list' ? 'btn-primary active' : 'btn-outline-secondary' }} px-3 py-1 fs-12 fw-semibold">
                            <i class="feather-list me-1"></i> {{ __('production.table_view') }}
                        </a>
                    </div>

                    {{-- Time Horizon Scale selection --}}
                    <div class="d-flex align-items-center gap-1">
                        <a href="{{ request()->fullUrlWithQuery(['view' => 'day']) }}" class="btn btn-sm {{ $view === 'day' ? 'btn-primary active' : 'btn-outline-secondary' }} px-3 py-1 fs-12 fw-semibold">{{ __('production.day') }}</a>
                        <a href="{{ request()->fullUrlWithQuery(['view' => 'week']) }}" class="btn btn-sm {{ $view === 'week' ? 'btn-primary active' : 'btn-outline-secondary' }} px-3 py-1 fs-12 fw-semibold">{{ __('production.week') }}</a>
                        <a href="{{ request()->fullUrlWithQuery(['view' => 'month']) }}" class="btn btn-sm {{ $view === 'month' ? 'btn-primary active' : 'btn-outline-secondary' }} px-3 py-1 fs-12 fw-semibold">{{ __('production.month') }}</a>
                    </div>
                </div>
            </div>
                    ...
                    <div class="gantt-chart-container border rounded overflow-hidden shadow-sm mb-4" style="overflow-x: auto;">
                                            ...
                                            @foreach($machine['lanes'] as $lane)
                                                @foreach($lane['operations'] as $op)
                                                    @php
                                                        ...
                                                        elseif ($op['status'] === 'completed') $barBg = 'bg-secondary text-white';
                                                        ...
                                                        $tooltipHtml = "<div class='text-start p-1' style='font-size:11px;line-height:1.45;'><div class='fw-bold border-bottom border-secondary pb-1 mb-1 text-white'>" . e($op['name']) . " (" . e($op['operation_number']) . ")</div>"
                                                                     . "<div><strong>Order:</strong> " . e($op['order_number']) . "</div>"
                                                                     . "<div><strong>Product:</strong> " . e($op['product_name']) . "</div>"
                                                                     . "<div><strong>Work Center:</strong> " . e($wc['name']) . "</div>"
                                                                     . "<div><strong>Start:</strong> " . e($op['planned_start'] ? $op['planned_start']->format('d/m H:i') : 'N/A') . "</div>"
                                                                     . "<div><strong>End:</strong> " . e($op['planned_finish'] ? $op['planned_finish']->format('d/m H:i') : 'N/A') . "</div>"
                                                                     . "<div class='mt-1 pt-1 border-top border-secondary'><strong>Status:</strong> <span class='badge bg-light text-dark'>" . ucfirst($op['status']) . "</span></div></div>";

                                                        if ($hasConflict) {
                                                            $tooltipHtml .= "<div class='text-danger mt-1 pt-1 border-top fw-bold'>Warnings:</div>";
                                                            foreach ($op['conflict_data']['conflicts'] as $c) {
                                                                $tooltipHtml .= "<div class='text-danger small'>• " . e($c['message']) . "</div>";
                                                            }
                                                        }
                                                    @endphp
                                                    @if($op['width_percent'] > 0)
                                                        <div class="position-absolute rounded shadow-sm text-truncate px-2 py-1 fs-11 font-monospace fw-bold {{ $barBg }} {{ $borderClass }} d-flex align-items-center justify-content-between cursor-pointer"
                                                             style="left: {{ $op['left_percent'] }}%; width: {{ max(2.5, $op['width_percent']) }}%; top: {{ 10 + ($op['visual_lane'] * 46) }}px; height: 36px; z-index: 5;"
                     ...
                     <div class="d-flex flex-wrap align-items-center justify-content-between p-3 bg-light rounded border fs-12 mt-4">
                         <div class="d-flex flex-wrap align-items-center gap-3">
                             ...
                             <span class="d-flex align-items-center gap-1"><span class="d-inline-block rounded-circle bg-warning" style="width: 10px; height: 10px;"></span> In Progress</span>
                             ...
                             <span class="d-flex align-items-center gap-1"><span class="badge bg-soft-danger text-danger border border-danger fs-10">Paused</span></span>
                             <span class="d-flex align-items-center gap-1"><i class="feather-alert-triangle text-warning"></i> Conflict Warning</span>
                         </div>
                         <div class="text-muted fs-11 mt-1 mt-md-0">
                             Hover over operation bars to view order, product, work center, and schedule details.
                         </div>
                     </div>
                @else
