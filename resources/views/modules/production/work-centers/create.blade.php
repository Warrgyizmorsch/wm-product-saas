...
@section('content')
    <div class="erp-single-panel bg-white">
        ...
        <div class="d-flex justify-content-between align-items-center mb-4 pb-3 border-bottom">
            ...
            <a href="{{ route('production.work-centers.index') }}" class="text-muted hover-danger fs-18">
                <i class="feather-x"></i>
            </a>
        </div>
        ...
        <form method="POST" action="{{ route('production.work-centers.store') }}">
            ...
            <div class="row g-4">
                ...
                <div class="col-md-6">
                    ...
                    <x-ui.input label="Work Center Code*" name="code" placeholder="e.g. WC-ASSY-A" value="{{ old('code') }}" required />
                    ...
                    ]" selected="{{ old('status', 'active') }}" data-select2-selector="default" required />
                </div>

                <!-- Right Column -->
                <div class="col-md-6">
                    <x-ui.input label="Capacity Per Hour" name="capacity_per_hour" type="number" step="any" placeholder="e.g. 50.00" value="{{ old('capacity_per_hour') }}" helperText="Leave empty for unlimited/flexible capacity" />
                    
                    <x-ui.input label="Efficiency (%)*" name="efficiency_percentage" type="number" step="any" placeholder="100.00" value="{{ old('efficiency_percentage', '100.00') }}" required />
                    
                    <x-ui.input label="Cost Per Hour ($)*" name="cost_per_hour" type="number" step="any" placeholder="0.0000" value="{{ old('cost_per_hour', '0.0000') }}" required />
                    
                    <x-ui.textarea label="Description" name="description" placeholder="Enter purpose, operational limits, or other description..." value="{{ old('description') }}" rows="4" />
                </div>
            </div>
