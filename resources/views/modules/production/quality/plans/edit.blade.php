...
@section('title', 'Edit Quality Plan | SaaS ERP')
@section('page-title', 'Edit Quality Control Plan')
@section('breadcrumb', 'Edit Plan')
...
@section('content')
    <div class="erp-single-panel bg-white" x-data="qualityPlanForm()">
        ...
        <form method="POST" action="{{ route('production.quality-plans.update', $plan->id) }}">
            @csrf
            @method('PUT')
            ...
            <x-ui.odoo-form-ui type="sheet">
                ...
                <div class="d-flex justify-content-between align-items-center mb-4 pb-3 border-bottom">
                    <h4 class="fw-bold text-dark mb-0">Edit Quality Plan - {{ $plan->name }}</h4>
                ...
                <div class="row g-4 mb-4 fs-13 text-dark">
                    <div class="col-md-6">
                        <x-ui.odoo-form-ui type="input" label="Plan Name" name="name" placeholder="e.g. Standard Final Quality Checklist" :value="old('name', $plan->name)" :required="true" :error-text="$errors->first('name')" />
                        <x-ui.odoo-form-ui type="input" label="Version" name="version" placeholder="e.g. 1.0" :value="old('version', $plan->version)" :required="true" :error-text="$errors->first('version')" />
                        ...
                        <div x-show="scopeType === 'product'" x-transition class="mb-3">
                            <x-ui.odoo-form-ui type="select" label="Target Product" name="product_id" id="product_id" :error-text="$errors->first('product_id')">
                                ...
                                @foreach($products as $p)
                                    <option value="{{ $p->id }}" @selected($p->id == $plan->product_id)>{{ $p->sku }} - {{ $p->name }}</option>
                                @endforeach
                            </x-ui.odoo-form-ui>
                        </div>
                        ...
                        <div x-show="scopeType === 'work_center'" x-transition class="mb-3">
                            <x-ui.odoo-form-ui type="select" label="Target Work Center" name="work_center_id" id="work_center_id" :error-text="$errors->first('work_center_id')">
                                ...
                                @foreach($workCenters as $wc)
                                    <option value="{{ $wc->id }}" @selected($wc->id == $plan->work_center_id)>{{ $wc->name }}</option>
                                @endforeach
                            </x-ui.odoo-form-ui>
                        </div>

                        <x-ui.odoo-form-ui type="select" label="Approval Status" name="status" id="status" :error-text="$errors->first('status')">
                            <option value="draft" @selected($plan->status === 'draft')>{{ __('production.draft_schedules') }}</option>
                            <option value="submitted" @selected($plan->status === 'submitted')>{{ __('production.submitted') }}</option>
                            <option value="approved" @selected($plan->status === 'approved')>Approved & Active</option>
                        </x-ui.odoo-form-ui>
                    </div>
                </div>
                ...
                <div class="d-flex justify-content-end gap-2 mt-4 pt-3 border-top">
                    ...
                    <button type="submit" class="btn btn-primary">Update Quality Plan</button>
                </div>
            </x-ui.odoo-form-ui>
        </form>
    </div>
...
@push('scripts')
    ...
    <script>
        function qualityPlanForm() {
            return {
                scopeType: '{{ $plan->type }}',
                parameters: @json($plan->parameters),
                init() {
                    // Normalize is_mandatory to boolean
                    this.parameters = this.parameters.map(p => {
                        p.is_mandatory = !!p.is_mandatory;
                        return p;
                    });
                },
                addParameter() {
