                ...
                <form method="GET" action="{{ route('production.rework.index') }}" class="d-inline">
                    <x-ui.filter label="Filter" offset="0, 5">
                        ...
                        <div class="d-flex gap-2 justify-content-end mt-4">
                            <a href="{{ route('production.rework.index') }}" class="btn btn-sm btn-light border">{{ __('production.reset') }}</a>
        ...
        <x-ui.odoo-form-ui type="table">
            <thead>
                <tr>
                    ...
                    <th style="width: 12%" class="text-end">{{ __('production.actual_cost') }}</th>
                    ...
                    <tr>
                        ...
                        <td>
                            @if($rwk->status === 'completed')
                                <span class="erp-badge-active">{{ __('production.completed_schedules') }}</span>
                            @elseif($rwk->status === 'failed')
                                <span class="badge bg-soft-danger text-danger text-uppercase px-2 py-1 fs-10">{{ __('production.failed') }}</span>
                            @elseif($rwk->status === 'running')
                                <span class="erp-badge-pending">{{ __('production.running') }}</span>
                            @else
