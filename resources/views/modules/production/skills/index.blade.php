                ...
                <form method="GET" action="{{ route('production.operator-skills.index') }}" class="d-inline">
                    <x-ui.filter label="Filter" offset="0, 5">
                        ...
                        <div class="d-flex gap-2 justify-content-end mt-4">
                            <a href="{{ route('production.operator-skills.index') }}" class="btn btn-sm btn-light border">{{ __('production.reset') }}</a>
            ...
            <tbody>
                @forelse($skills as $sk)
                    <tr>
                        ...
                        <td>
                            @if($sk->active)
                                <span class="erp-badge-active">{{ __('production.status_active') }}</span>
                            @else
                                <span class="erp-badge-draft">{{ __('production.wc_status_inactive') }}</span>
                            @endif
                        </td>
                        <td class="text-end">
                            <x-ui.action-dropdown>
                                <li>
                                    <a href="{{ route('production.operator-skills.edit', $sk->id) }}" class="dropdown-item">
                                        <i class="feather-edit me-2 text-muted fs-12"></i>Edit Skill
                                    </a>
                                </li>
                                <li><hr class="dropdown-divider"></li>
                                <li>
                                    <form method="POST" action="{{ route('production.operator-skills.destroy', $sk->id) }}"
                                          onsubmit="return confirm('Are you sure you want to delete this skill mapping?');">
                                        @csrf
                                        @method('DELETE')
