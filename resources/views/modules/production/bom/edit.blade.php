        ...
        <form method="POST" action="{{ route('production.boms.update', $bom->id) }}">
            ...
            <x-ui.odoo-form-ui type="sheet">
                ...
                <div class="d-flex justify-content-between align-items-center mb-4 pb-2 border-bottom">
                    <h4 class="fw-bold text-dark mb-0">{{ __('production.edit_bill_of_materials_with_number', ['number' => $bom->bom_number]) }}</h4>
                    <a href="{{ route('production.boms.show', $bom->id) }}" class="btn btn-sm btn-light border">{{ __('production.cancel') }}</a>
                </div>
                ...
                <div class="row g-4 mb-4 fs-13 text-dark">
                    ...
                    <div class="col-md-6 border-end">
                        <x-ui.odoo-form-ui type="input" :label="__('production.bom_number')" name="bom_number" placeholder="e.g. BOM-XYZ-001" :value="old('bom_number', $bom->bom_number)" :required="true" :error-text="$errors->first('bom_number')" alpineError="errors.bom_number" />
                        ...
                        <x-ui.odoo-form-ui type="input" :label="__('production.bom_name')" name="bom_name" placeholder="e.g. Standard Red Door BOM" :value="old('bom_name', $bom->bom_name)" :required="true" :error-text="$errors->first('bom_name')" alpineError="errors.bom_name" />
                        ...
                        <x-ui.odoo-form-ui type="select" :label="__('production.bom_type')" name="bom_type" :required="true" :error-text="$errors->first('bom_type')" alpineError="errors.bom_type">
                            <option value="manufacturing" @selected(old('bom_type', $bom->bom_type) === 'manufacturing')>{{ __('production.bom_type_manufacturing') }}</option>
                            <option value="engineering" @selected(old('bom_type', $bom->bom_type) === 'engineering')>{{ __('production.bom_type_engineering') }}</option>
                            <option value="sales" @selected(old('bom_type', $bom->bom_type) === 'sales')>{{ __('production.bom_type_sales') }}</option>
                            <option value="phantom" @selected(old('bom_type', $bom->bom_type) === 'phantom')>{{ __('production.bom_type_phantom') }}</option>
                            <option value="subcontracting" @selected(old('bom_type', $bom->bom_type) === 'subcontracting')>{{ __('production.bom_type_subcontracting') }}</option>
                        </x-ui.odoo-form-ui>
                        ...
                        <x-ui.odoo-form-ui type="select" :label="__('production.usage_context')" name="usage_context" :required="true" :error-text="$errors->first('usage_context')" alpineError="errors.usage_context">
                            <option value="manufacturing" @selected(old('usage_context', $bom->usage_context) === 'manufacturing')>{{ __('production.usage_context_manufacturing') }}</option>
                            <option value="engineering" @selected(old('usage_context', $bom->usage_context) === 'engineering')>{{ __('production.usage_context_engineering') }}</option>
                            <option value="prototype" @selected(old('usage_context', $bom->usage_context) === 'prototype')>{{ __('production.usage_context_prototype') }}</option>
                            <option value="costing" @selected(old('usage_context', $bom->usage_context) === 'costing')>{{ __('production.usage_context_costing') }}</option>
                        </x-ui.odoo-form-ui>
                    </div>

                    <!-- Right Column -->
                    <div class="col-md-6">
                        <x-ui.odoo-form-ui type="input" :label="__('production.base_quantity')" name="base_quantity" inputType="number" step="any" placeholder="1.0" :value="old('base_quantity', $bom->base_quantity)" :required="true" :error-text="$errors->first('base_quantity')" alpineError="errors.base_quantity" />
                        ...
                        <x-ui.odoo-form-ui type="textarea" :label="__('production.revision_notes')" name="notes" placeholder="Max. 500 characters" rows="3" :error-text="$errors->first('notes')" alpineError="errors.notes">{{ old('notes', $bom->notes) }}</x-ui.odoo-form-ui>
                    </div>
                </div>
                ...
                <div class="border rounded mb-4">
                    ...
                    <div class="bg-white" 
                        ...
                        <div class="row g-3 fs-13 text-dark">
                            <div class="col-md-6">
                                <x-ui.odoo-form-ui type="input" :label="__('production.version')" name="version" placeholder="e.g. 1.0.0" :value="old('version', $bom->version)" :required="true" :error-text="$errors->first('version')" alpineError="errors.version" />
                            </div>
                            <div class="col-md-6">
                            ...
                            <div class="col-md-6">
                                <x-ui.odoo-form-ui type="input" :label="__('production.start_date') ?? 'Start Date'" name="effective_date" inputType="date" :value="old('effective_date', $bom->effective_date ? $bom->effective_date->format('Y-m-d') : date('Y-m-d'))" :required="true" :error-text="$errors->first('effective_date')" alpineError="errors.effective_date" />
                            </div>
                            <div class="col-md-6">
                                <x-ui.odoo-form-ui type="input" :label="__('production.expiry_date') ?? 'Expiry Date'" name="expiry_date" inputType="date" :value="old('expiry_date', $bom->expiry_date ? $bom->expiry_date->format('Y-m-d') : '')" :error-text="$errors->first('expiry_date')" alpineError="errors.expiry_date" />
                            </div>
                            <div class="col-md-12">
                                <x-ui.odoo-form-ui type="input" :label="__('production.revision_reason') ?? 'Revision Reason'" name="revision_reason" placeholder="e.g. Engineering specification update" :value="old('revision_reason', $bom->revision_reason)" :error-text="$errors->first('revision_reason')" alpineError="errors.revision_reason" />
                            </div>
                        </div>
                    </div>
                </div>
                                            ...
                                            <td class="align-middle">
                                                <div class="d-flex flex-column gap-2">
                                                    <div>
                                                        ...
                                                        <x-ui.odoo-form-ui type="checkbox" name="items[][is_alternative]" x-model="item.is_alternative" x-bind:id="'is_alternative_edit_' + index">
                ...
                <div class="d-flex gap-2 pt-3 border-top mt-4">
                    ...
                    <a href="{{ route('production.boms.show', $bom->id) }}" class="btn btn-secondary px-4">{{ __('production.cancel') }}</a>
                </div>
            </x-ui.odoo-form-ui>
        </form>
    </div>
