        ...
        <form action="{{ $action }}" method="POST" @isset($formId) id="{{ $formId }}" @endisset>
            ...
            @if ($isModal)
                <x-ui.odoo-form-ui type="sheet" class="border-0 shadow-none p-0 mb-0">
                    @include('modules.platform.plans.partials.form-fields', [
                        'plan' => $plan,
                    ])
            ...
            @else
                <x-ui.card title="Plan Details" class="border-0 shadow-sm">
                    @include('modules.platform.plans.partials.form-fields', [
                        'plan' => $plan,
                    ])
                    ...
                    <x-slot name="footer">
                        <div class="d-flex justify-content-end gap-2">
                            <x-ui.button href="{{ route('platform.plans.index') }}" variant="light">Cancel</x-ui.button>
                            <x-ui.button type="submit" variant="primary" icon="feather-check-circle">{{ $submitLabel }}</x-ui.button>
                        </div>
                    </x-slot>
                </x-ui.card>
