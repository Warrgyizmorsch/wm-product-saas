...
@section('title', 'AP Aging | SaaS ERP')
...
@section('content')
    <x-ui.card class="mb-4">
        <x-ui.filter-toolbar :resetUrl="route('accounting.reports.ap-aging')" searchLabel="View">
    ...
    <x-ui.card title="Outstanding by Age" bodyClass="p-0" class="accounting-dense">
        <x-ui.table hoverable>
            ...
            <tbody class="fs-13 text-dark">
                ...
                @empty
                    <tr>
                        <td colspan="7" class="text-center py-4 text-muted">No outstanding payables.</td>
                    </tr>
                @endforelse
            </tbody>
            <tfoot>
    ...
    @if (count($vendors))
        <x-ui.card title="Bill Detail" bodyClass="p-0" class="accounting-dense mt-4">
            <x-ui.table hoverable>
                <thead class="table-light fs-11 text-uppercase fw-semibold text-muted">
                    <tr>
                        <th class="ps-4">Vendor</th>
                    ...
                    @foreach ($vendors as $row)
                        @foreach ($row['bills'] as $item)
                            <tr>
                                ...
                                <td class="font-monospace">{{ $item['bill']->bill_number }}</td>
