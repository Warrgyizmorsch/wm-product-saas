...
@section('content')
    @php
        ...
        $buckets = array_values(\App\Domains\Accounting\Services\Dashboard\PartyBalances::BUCKETS);
        $charts = [
            'trend' => $trend,
            'receivables' => ['labels' => $buckets, 'values' => array_values($receivables['buckets'])],
            'payables' => ['labels' => $buckets, 'values' => array_values($payables['buckets'])],
    ...
    <ul class="nav nav-tabs mb-3">
        @foreach ($views as $value => $label)
            <li class="nav-item">
                <a class="nav-link {{ $activeView === $value ? 'active' : '' }}" href="{{ route('accounting.dashboard', ['view' => $value] + $query) }}">
                    <i class="{{ $value === 'overview' ? 'feather-activity' : 'feather-check-square' }} me-1"></i>{{ $label }}
                    @if ($value === 'operations' && $checklistDone < count($checklist))
                        <span class="badge bg-soft-warning text-warning ms-1">{{ count($checklist) - $checklistDone }}</span>
                    @endif
                </a>
            </li>
        @endforeach
