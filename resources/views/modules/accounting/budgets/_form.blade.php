...
<x-ui.odoo-form-ui type="sheet">
    ...
    @if ($embedded)
        <div class="row g-3 fs-13 text-dark">
            <div class="col-md-6">
                <label class="form-label fw-semibold fs-13 text-dark mb-2">Fiscal Year <span class="text-danger">*</span></label>
                <x-ui.select name="fiscal_year_id" :required="true" :selected="old('fiscal_year_id', $budget?->fiscal_year_id)" :options="['' => 'Select Fiscal Year...'] + $fiscalYears->mapWithKeys(fn ($fy) => [$fy->id => $fy->name . ' (' . $fy->status . ')'])->all()" />
            </div>
            <div class="col-md-6">
                <x-ui.icon-input label="Budget Name" icon="feather-tag" name="name" :value="old('name', $budget?->name)" placeholder="e.g. FY26 Operating Budget" :required="true" />
            </div>
        </div>
    @else
            ...
            <x-ui.odoo-form-ui type="table" id="itemsTable">
                <thead>
                    <tr>
                        ...
                        <th style="width: 21%;">Cost Center / Department / Project</th>
                        <th class="text-end" style="width: 15%;">Annual Amount</th>
