...
@section('title', __('purchase.po_approvals') . ' | SaaS ERP')
@section('page-title', __('purchase.po_approvals'))
@section('breadcrumb', __('ui.purchase') . ' / ' . __('purchase.po_approvals'))
...
@section('content')
    ...
    <div class="erp-single-panel">
        ...
        <div class="d-flex align-items-center mb-3">
            <div>
                <h5 class="fw-bold text-dark mb-0">{{ __('purchase.po_approvals') }}</h5>
                <p class="text-muted fs-12 mb-0">{{ __('purchase.po_approvals_help') }}</p>
            </div>
            ...
            <div class="d-flex gap-2 ms-auto">
                ...
                <form method="GET" action="{{ route('purchase.po-approvals.index') }}" class="d-inline">
                    <x-ui.filter :label="__('ui.filter') ?? 'Filters'" offset="0, 5">
                        <h6 class="fw-bold text-dark fs-12 mb-3"><i class="feather-sliders me-1 text-primary"></i> {{ __('purchase.filter') }}</h6>
                        <div class="mb-3">
                        ...
                        <div class="d-flex gap-2 justify-content-end mt-4">
                            <a href="{{ route('purchase.po-approvals.index') }}" class="btn btn-sm btn-light border">{{ __('purchase.reset') }}</a>
        ...
        <div class="d-flex gap-2 mb-3">
            <span class="badge bg-warning-subtle text-warning border border-warning-subtle fs-12 px-3 py-2 rounded-pill">
                ...
                {{ $orders->total() }} {{ $orders->total() == 1 ? __('purchase.pending_approval_badge') : __('purchase.pending_approvals_badge') }}
            </span>
        </div>
            ...
            <x-ui.odoo-form-ui type="table" id="poApprovalTable">
                <thead>
                    <tr>
                        ...
                        <th style="width: 15%">{{ __('purchase.dates') }}</th>
                        ...
                        <tr>
                            ...
                            <td class="text-center">
                                ...
                                <x-ui.badge :soft="true" :variant="$statusClass">
                                    {{ __('purchase.status_' . strtolower(str_replace(' ', '_', $order->status))) ?: $order->status }}
                                </x-ui.badge>
                            </td>
                            <td class="text-end" style="white-space: nowrap;">
                                <div class="d-inline-flex align-items-center gap-1 justify-content-end">
                                    ...
                                    <form action="{{ route('purchase.orders.approve', $order->id) }}" method="POST" class="d-inline-flex flex-shrink-0" id="approvePoForm_{{ $order->id }}">
                                        @csrf
                                        <button type="button" class="action-dropdown-btn action-btn-approve" title="{{ __('purchase.approve') }}" data-bs-toggle="tooltip" onclick="confirmAction({ title: '{{ __('purchase.approve_po') }}', message: '{{ __('purchase.confirm_approve_po') }}', variant: 'success', confirmText: '{{ __('purchase.approve') }}' }, function() { document.getElementById('approvePoForm_{{ $order->id }}').submit(); })">
                                            <i class="feather feather-check-circle"></i>
                                        </button>
                                    </form>

                                     {{-- Reject --}}
                                     <button type="button" class="action-dropdown-btn action-btn-reject flex-shrink-0" title="{{ __('purchase.reject') }}" data-bs-toggle="tooltip" onclick="openRejectModal('{{ route('purchase.orders.reject', $order->id) }}', '{{ $order->purchase_order_number }}')">
                    ...
                    @empty
                        <tr>
                            <td colspan="9" class="text-center py-5 text-muted fs-14">
                                ...
                                {{ __('purchase.no_po_pending_approval') }}
                            </td>
                        </tr>
                    @endforelse
                </tbody>
    ...
    <x-ui.drawer id="poApprovalDrawer" :title="__('purchase.purchase_order_details')" position="end" style="width: 580px; max-width: 95vw;">
            ...
            <form id="rejectActionForm" method="POST" action="">
                ...
                <div class="modal-header bg-soft-danger text-danger border-bottom-0">
                    <h5 class="modal-title fw-bold" id="rejectActionModalLabel">
                        <i class="feather-x-circle me-2"></i>{{ __('purchase.reject_purchase_order') }} <span id="rejectModalDocNumber" class="text-dark"></span>
                    </h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body p-4">
                    <p class="text-muted fs-12 mb-3">{{ __('purchase.reject_po_help_text') }}</p>
                    ...
                    <div class="mb-3 text-start">
                        ...
                        <textarea class="form-control" id="rejectionReasonInput" name="rejection_reason" rows="4" placeholder="{{ __('purchase.reject_po_reason_placeholder') }}" required></textarea>
                    </div>
                </div>
