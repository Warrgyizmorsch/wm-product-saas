    ...
    <div class="erp-single-panel bg-white p-4 shadow-sm rounded border-0 text-dark">
            ...
            <div class="alert alert-info border-info p-3 mb-4 rounded shadow-sm">
                <div class="d-flex align-items-center justify-content-between flex-wrap gap-2">
                    <div>
                        ...
                        <span class="text-success fw-bold font-monospace fs-14 ms-1">{{ format_currency($availAdv) }}</span>
                    ...
                    <div class="d-flex align-items-center gap-2">
                        ...
                        @endif
                        <span class="badge bg-primary text-white p-2 fs-12">{{ __('purchase.net_payable_from_bank') }}: {{ format_currency(max(0, $bill->due_amount - $availAdv)) }}</span>
                    </div>
                </div>
            </div>
        @endif
        ...
        <div class="row g-3 mb-4 fs-13 text-dark border-bottom pb-3">
            ...
            @else
                ...
                <div class="col-md-2">
                    ...
                    @if($bill->freight_amount > 0)
                        <span class="text-primary fw-bold font-monospace d-inline-block">{{ format_currency($bill->freight_amount) }}</span>
            ...
            <div class="col-md-2 text-md-end">
                ...
                <strong class="fs-16 font-monospace text-primary">{{ format_currency($bill->grand_total) }}</strong>
            </div>
        </div>
            ...
            <table class="table table-bordered table-sm align-middle fs-13 text-dark mb-0">
                <thead class="table-light fs-11 text-uppercase text-muted fw-semibold">
                    <tr>
                        ...
                        <th class="text-end" style="width: 12%;">Amount ({{ active_currency_symbol() }})</th>

                        @if($isProRata)
                            <th class="text-end text-primary" style="width: 12%;">Freight Share ({{ active_currency_symbol() }})</th>
                        @endif
                ...
                <tbody>
                    @foreach($bill->items as $idx => $item)
                        ...
                        <tr>
                            ...
                            <td class="text-end font-monospace">{{ format_currency($rate) }}</td>
                            <td class="text-end font-monospace">{{ format_currency($lineSub) }}</td>

                            @if($isProRata)
                                <td class="text-end font-monospace text-primary fw-semibold">+{{ format_currency($itemFreightShare) }}</td>
                            @endif

                            <td class="text-center font-monospace">{{ $taxRate }}%</td>
                            <td class="text-end font-monospace text-muted">{{ format_currency($lineTax) }}</td>
                            <td class="text-end pe-3 font-monospace fw-bold text-dark">{{ format_currency($lineTotal) }}</td>
                        </tr>
                    @endforeach
                </tbody>
            </table>
            ...
            <div class="col-md-6 offset-md-6">
                <div class="border rounded p-3 bg-light-50 fs-13 text-dark">
                    <div class="d-flex justify-content-between mb-2">
                        ...
                        <strong class="font-monospace">{{ format_currency($bill->subtotal) }}</strong>
                    </div>

                    @if($bill->discount_amount > 0)
                        <div class="d-flex justify-content-between mb-2 text-danger">
                            <span>{{ __('purchase.discount') }}:</span>
                            <strong class="font-monospace">-{{ format_currency($bill->discount_amount) }}</strong>
                        </div>
                        <div class="d-flex justify-content-between mb-2 border-top pt-1">
                            <span class="text-muted">Items Taxable Value:</span>
                            <strong class="font-monospace">{{ format_currency(max(0, $bill->subtotal - $bill->discount_amount)) }}</strong>
                        </div>
                    @endif
                    ...
                    @if($bill->freight_amount > 0 && !$isProRata && $bill->tax_type !== 'order_wise_tax')
                        <div class="d-flex justify-content-between mb-2 text-primary">
                            ...
                            <strong class="font-monospace">+{{ format_currency($bill->freight_amount) }}</strong>
                        </div>
                    ...
                    @elseif($bill->freight_amount > 0 && $bill->tax_type === 'order_wise_tax')
                        ...
                        <div class="d-flex justify-content-between mb-2 border-top pt-1 fw-bold">
                            ...
                            <strong class="font-monospace">{{ format_currency(max(0, $bill->subtotal - $bill->discount_amount) + $bill->freight_amount) }}</strong>
                        </div>
                    @endif
                    ...
                    @if($bill->gst_type === 'igst')
                        <div class="d-flex justify-content-between mb-2 text-muted">
                            ...
                            <strong class="font-monospace">+{{ format_currency($displayIgst) }}</strong>
                        </div>
                    @else
                        <div class="d-flex justify-content-between mb-2 text-muted">
                            <span>CGST (Central Tax):</span>
                            <strong class="font-monospace">+{{ format_currency($displayCgst) }}</strong>
                        </div>
                        <div class="d-flex justify-content-between mb-2 text-muted">
                            <span>SGST (State Tax):</span>
                            <strong class="font-monospace">+{{ format_currency($displaySgst) }}</strong>
                        </div>
                    @endif
                    ...
                    <div class="d-flex justify-content-between fw-bold fs-15 text-primary">
                        <span>{{ __('purchase.grand_total') }}:</span>
                        <strong class="font-monospace">{{ format_currency($bill->grand_total) }}</strong>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="row g-3 mb-4">
            <div class="col-md-4">
                <div class="p-3 border rounded bg-light-50">
                    ...
                    <h4 class="fw-bold text-dark mb-0">{{ format_currency($bill->grand_total) }}</h4>
                </div>
            </div>
            <div class="col-md-4">
                <div class="p-3 border rounded bg-light-50">
                    <span class="fs-11 text-uppercase text-success fw-bold d-block mb-1">{{ __('purchase.paid_settled_amount') }}</span>
                    <h4 class="fw-bold text-success mb-0">{{ format_currency($bill->paid_amount) }}</h4>
                </div>
            </div>
            <div class="col-md-4">
                <div class="p-3 border rounded bg-light-50">
                    <span class="fs-11 text-uppercase text-danger fw-bold d-block mb-1">{{ __('purchase.net_balance_due') }}</span>
                    <h4 class="fw-bold text-danger mb-0">{{ format_currency($bill->due_amount) }}</h4>
                </div>
            </div>
        </div>
            ...
            <div class="alert alert-info border-info p-3 mb-4 rounded shadow-sm">
                <div class="d-flex align-items-start gap-2">
                    ...
                    <div class="w-100">
                        ...
                        <p class="mb-2 fs-12 text-muted">Base freight of <strong>{{ format_currency($bill->landed_cost_revaluation_data['total_base_freight'] ?? $bill->freight_amount) }}</strong> was allocated to GRN received items. Effective Landed Stock Rate updated in Stock Ledger:</p>
                                ...
                                <tbody>
                                    @foreach($bill->landed_cost_revaluation_data['revaluation_items'] as $revItem)
                                        <tr>
                                            ...
                                            <td class="text-end font-monospace">{{ format_currency($revItem['base_unit_cost']) }}</td>
                                            <td class="text-end font-monospace text-success">+{{ format_currency($revItem['freight_share']) }}</td>
                                            <td class="text-end font-monospace text-success">+{{ format_currency($revItem['freight_per_unit']) }}</td>
                                            <td class="text-end font-monospace fw-bold text-primary">{{ format_currency($revItem['new_landed_cost']) }}</td>
                                        </tr>
                                    @endforeach
                                </tbody>
                            </table>
